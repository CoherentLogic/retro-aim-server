...
type Config struct {
	...
	OSCARListenHost string `envconfig:"OSCAR_LISTEN_HOST" required:"true" val:"0.0.0.0" description:"Specifies the IP address that the TCP listener should bind to. Setting this to '0.0.0.0' allows the listener to accept connections on all IPv4 interfaces of the host."`
	OSCARClientHost string `envconfig:"OSCAR_CLIENT_HOST" required:"true" val:"127.0.0.1" description:"The hostname that AIM clients connect to in order to reach OSCAR services (auth, BOS, BUCP, etc). Make sure the hostname is reachable by all clients. For local development, the default loopback address should work provided the server and AIM client(s) are running on the same machine. For LAN-only clients, a private IP address (e.g. 192.168..) or hostname should suffice. For clients connecting over the Internet, specify your public IP address and ensure that TCP ports 5190-5194 are open on your firewall."`
}
